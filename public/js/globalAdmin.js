...
//------------------------------------------------------------------------------
    var CONFIG = window.globalAdminConfig;
    
    window.globalAdmin        = function(element, options){
//------------------------------------------------------------------------------
//----------------PROTOTYPE VARIABLES-------------------------------------------
//------------------------------------------------------------------------------
        this.$el                = null;
...
//------------------------------------------------------------------------------
        var _init = function(){
            ...
            $('body').data('globalAdmin', me);
            if ( $(window).width() <= globalAdmin.SCREEN_MD){
            ...
            _initSettingBox();
            _loadOnHashChange();
            $(window).trigger('hashchange.globalAdmin');
        };
        var _enableResponsiveSearchForm = function(){
            $(CONFIG.searchFormSelector).find('.btn-search').on('click.globalAdmin', function(ev){
                ev.preventDefault();
                ev.stopPropagation();
                $(this).closest('.navbar-search').addClass('navbar-search-full')
                    .find('input')[0].focus();
            });
            $(CONFIG.searchFormSelector).find('.btn-remove').on('click.globalAdmin', function(ev){
                ev.preventDefault();
                ev.stopPropagation();
        ...
        var _disableResponsiveSearchForm = function(){
            $(CONFIG.searchFormSelector).find('.btn-search, .btn-remove').off('click.globalAdmin');
        };
        var _enableResponsiveness = function(){
            $(window).on('resize.globalAdmin', function(){
               if ( $(window).width() <= globalAdmin.SCREEN_MD){
        ...
        var _initNavigation = function () {
            ...
            pan.data('globalAdmin.navMenu', menu);
            return menu;
        };
        var _triggerEvent = function (eventType) {
            var args = Array.prototype.slice.call(arguments, 1);
            args.unshift(me);
            me.$el.trigger(eventType+'.globalAdmin', args);
        };
        var _readLocalStorage = function(){
            ...
            var config = null, 
                fromStorage = localStorage.getItem('globalAdmin');
            try {
                if (fromStorage) {
                    config = JSON.parse(fromStorage);
                }
            } catch (e) {
                window.console.error('"globalAdmin" localStorage object is not valid JSON');
            }
            if (config){
        ...
        var _putToLocalStorage = function(config){
            if ($.isEmptyObject(config)){
                ...
                localStorage.setItem('globalAdmin', JSON.stringify(config));
            }
        };
        ...
        var _loadOnHashChange = function(){
            $(window).on('hashchange.globalAdmin', function(ev) {
                ...
                if ( ! me.$navPanel.isCollapsed() && $(window).width() < globalAdmin.SCREEN_MD) {
                    me.$navPanel.hide();
                }
        ...
        var _rewriteUrl = function(url){
            ...
            if (globalAdminRoutes[url]){
                return globalAdminRoutes[url];
            }
            //look through all routes and find by regexp if such router exists
            for (var i in globalAdminRoutes){
//                window.console.log("===============================================");
                var route = globalAdminRoutes[i];
...
//------------------------------------------------------------------------------
     ...
     * @returns {void}
     */
    globalAdmin.loadScript = function(srcs, callback){
     ...
     * @returns {void}
     */
    globalAdmin.loadCSS = function(href, callback){
     ...
     * @returns {void}
     */
    globalAdmin.highlightCode = function(){
     ...
     * @returns {String}
     */
    globalAdmin.lightenColor = function(col, amt) {
        var usePound = false;
     ...
     * @returns {String}
     */
    globalAdmin.fadeOutColor = function(col, amt){
        if (col[0] === "#") {
            col = col.slice(1);
        }
        //If color is HEX
...
//------------------------------------------------------------------------------
    ...
    var NavigationPanel = function(panel, globalAdmin){
//------------------------------------------------------------------------------
//----------------PROTOTYPE VARIABLES-------------------------------------------
//------------------------------------------------------------------------------
        this.$el                = panel;
...
//------------------------------------------------------------------------------
        ...
        var _init = function(){
            ...
            me.$menu.find('ul>li>a')
                    .off('click.globalAdmin')
                    .on('click.globalAdmin', function(e) {
...
//            _enableResponsiveness();
        ...
        var _preventEmptyLinksClick = function(){
            me.$menu.find('a[href="#"]').on('click.globalAdmin', function(ev){
                ev.preventDefault();
            });
        };
...
//        };
        var _hideMenuOnOutsideClick = function(){
            $(document).off('mouseup.globalAdmin')
                    .on('mouseup.globalAdmin', function(e){
...
//                    If this menu item is parent menu item
        ...
        var _enableHideShow = function(){
            me.$showHideBtn.on('click.globalAdmin', function(ev){
                ev.preventDefault();
                me.hideShow();
            });
        };
        ...
        var _enableToggle = function(){
            $(document).on('click.globalAdmin', CONFIG.sidebarCollapseExpandSelector, function(){
                me.toggle();
            });
        };
        var _triggerEvent = function (eventType) {
            var args = Array.prototype.slice.call(arguments, 1);
            args.unshift(me.$globalAdmin);
            me.$globalAdmin.$el.trigger(eventType + '.globalAdmin', args);
        };
//------------------------------------------------------------------------------
//----------------PROTOTYPE FUNCTIONS-------------------------------------------
//------------------------------------------------------------------------------
...
//                parent.parent().addClass('active');
        ...
        this.isHidden = function(){
            if ($('body').hasClass('menu-hidden') && $(window).width() >= globalAdmin.SCREEN_MD
                || ! $('body').hasClass('menu-hidden') && $(window).width() < globalAdmin.SCREEN_MD){
                return true;
            }
            return  false;
        };
...
//------------------------------------------------------------------------------
    globalAdmin.SCREEN_XS = 480;
    globalAdmin.SCREEN_SM = 768;
    globalAdmin.SCREEN_MD = 992;
    globalAdmin.SCREEN_LG = 1200;
    
//------------------------------------------------------------------------------
    $.fn.globalAdmin = function(option){
        return this.each(function(index, el){
            var $this = $(this);
            var data = $this.data('globalAdmin');
            var options = typeof option === 'object' && option;

            if (!data) {
                $this.data('globalAdmin', (data = new window.globalAdmin($this, options)));
            }
